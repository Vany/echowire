#!/usr/bin/env python3
"""
Download and convert ML models for EchoWire speech recognition service.
...
def download_embedding_model():
    ...
    download_file(EMBEDDINGS_URLS["model"], model_path, "all-MiniLM-L6-v2 ONNX model")

    download_file(EMBEDDINGS_URLS["tokenizer"], tokenizer_path, "tokenizer.json")
...
def verify_models(model_size: str):
    ...
    if all_exist:
        print("\n✅ All models verified:")
        print(
            f"   Whisper {model_size}: {whisper_path.stat().st_size / (1024 * 1024):.1f} MB"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and convert ML models for EchoWire service"
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--skip-embeddings", action="store_true", help="Skip embeddings model download"
    )

    args = parser.parse_args()

    print("=" * 60)
    print("EchoWire Model Download & Conversion Tool")
    print("=" * 60)
    ...
    try:
        ...
        if verify_models(args.model):
            ...
            print(
                "\n⚠ Some models are missing. Please check errors above.",
                file=sys.stderr,
            )
            return 1
